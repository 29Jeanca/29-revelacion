...
const SearchGuest = () =>{
    ...
    const searchName = async () =>{
         ...
         if(!users) return alert("No se pudo cargar la lista de invitados, intentalo mas tarde")
            const user = guest.find(user => user.nombre.toLowerCase().trim() === fullName.toLowerCase().trim());
         if(user){
    ...
    return(
        <>
            ...
            <input placeholder="Nombre Completo" className="input-searchGuest" onChange={(e)=>setFullName(e.target.value)}/>
            <button onClick={searchName} className="btn-searchGuest">Buscar</button>
